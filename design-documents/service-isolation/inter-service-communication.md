...
For a (existing) service such implementation will look like this:
```php
...
For a (new) service allowing asynchronous execution it would look like this:
```php
namespace Magento\ServiceProxy;

class ServiceAProxy implements \Magento\ServiceApi\ServiceAInterface
{
    /**
     * @param ADataInterface $aData
     * @return PromiseInterface Instance of create ADataInterface will be given to "then".
     */
    public function save(ADataInterface $aData): PromiseInterface
    {
        $promise = $this->invoker->invoke(
            \Magento\Service\ServiceAInterface::class,
            'save',
            [$aData]
        );
        $resultPromise = $this->promiseFactory->create();
        $promise->then(function (\Magento\Framework\Invoker\ResponseInterface $response) {
            if ($e = $response->getException()) {
                $resultPromise->reject($e);
            } else {
                $resultPromise->resolve($response->getResult());
            }
        });
...
We'd have to create new service contracts to allow asynchronous execution of __different__ service calls.
 
Example of client service:
```php
namespace Magento\Service;

class ServiceC implements \Magento\ServiceApi\ServiceCInterface
{
    public function doSmth(): SomeDTOInterface
    {
        ....
        
        $resultPromiseA = $this->serviceA->doAStuff();
        $resultPromiseB = $this->serviceB->doSomeOtherStuff();
        $resultPromiseA->wait();
        $resultPromiseB->wait();
...
```
In the example above ServiceA::doAStuff() and ServiceB::doSomeOtherStuff() are returning promises and are being executed
simultaneously and then after both are finished execution of ServiceC::doSmth() continues. 
...
services communications and invoker will always send GET requests to remote services. Services will accept any
HTTP method though so developers of services not based on Magento can send requests with methods they feel
suite a requests best. GET is chosen because some caching proxies like Varnish can only cache responses for
GET requests and such caching can be useful so we have to allow it.

While URLs for services will be autogenerated and GET always used to make it easier for Magento's invoker it will still
be possible for developers to write services without using Magento (or even PHP) - they would be able to get see URLs
for certain contracts with a CLI utility and while using GET for every request looks odd, functionally it wouldn't matter.
